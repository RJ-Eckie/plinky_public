...
const static float table_interp(const float *table,
...
static inline bool isgrainpreview(void) { return editmode == EM_SAMPLE; }
...
static inline bool isplaying(void) { return playmode == PLAYING || playmode == PLAY_WAITING_FOR_CLOCK_STOP; }
...
int ticks_since_step = 0; // this counts up, along with the seq_divide_counter, even when not playing, in order to give a sense of
                          // time eg for recording
...
static inline float lpf_k(int x) { return table_interp(lpf_ks, x); }
...
void update_params(int fingertrig, int fingerdown) {
  ...
  int axisswap =
      accel_raw[2] >
      4000; // run 2 plinkys have the accelerometer rotated 90 degrees and upside down from the addon... detect it via z direction
  for (int j = 0; j < 2; ++j) {
...
void RunVoice(Voice *v, int fingeridx, float targetvol, u32 *outbuf) {
  ...
  if (ramsample.samplelen) {
    ...
    if (trig) { // on trigger frames, we FADE out the old grains! then the next dma fetch will be the new sample and we can fade in
                // again
      EmuDebugLog("!!!!!!!!!!!!TRIG! %d\n", arpretrig);
      targetvol = 0.f;
      //		DebugLog("\r\n%d", fingeridx);
      ...
      float gdeadzone =
          clampf(minf(1.f - posjit, timestretch * 2.f), 0.f, 1.f); // if playing back normally and not jittering, add a deadzone
...
void processmidimsg(u8 msg, u8 d1, u8 d2) {
  ...
  case 0xf:            // system
    if (msg == 0xfa) { // start
      got_ui_reset = true;
      midi24ppqncounter = 5; // 2020-02-26: Used to be 0, changed to 5:
...
#define PITCH_BASE ((32768 - (12 << 9)) + (1 * 512) + 101)
  ...
  if (ramsample.samplelen > 0) {
    ...
    for (int i = 0; i < 8; ++i) {
      ...
      // TODO - if pos at end of next fetch will be out of bounds, negate dpos24 and grate_ratio so we ping pong back for the rest
      // of the grain!
...
#define MAX_SAMPLE_VOICES 6
  ...
  float g = param_eval_float(P_MIXHPF, any_rnd, env16,
...
int *EMSCRIPTEN_KEEPALIVE getemubitmap(void) { return (int *)emupixels; }
uint8_t *EMSCRIPTEN_KEEPALIVE getemuleds() { return (uint8_t *)led_ram; }
...
void set_test_rgb(int c) { set_test_mux(c ^ 7); }
...
*/
    ...
    // ok pdac[k][0] tells us what we got from the ADC when we set the DAC to PITCH_1V_OUT, and pdac[k][1] tells us what we got when
    // we output PITCH_4V_OUT
...
void EMSCRIPTEN_KEEPALIVE emu_setadc(float araw, float braw, float pitchcv, float gatecv, float xcv, float ycv, float acv,
                                     float bcv, int gateforce, int pitchsense, int gatesense) {
#ifdef EMU
...
bool send_midimsg(u8 status, u8 data1, u8 data2) { return true; }
void spi_update_dac(int chan) { resetspistate(); }
...
void EMSCRIPTEN_KEEPALIVE plinky_frame_wasm(void) { plinky_frame(); }
u32 wasmbuf[BLOCK_SAMPLES];
uint8_t *EMSCRIPTEN_KEEPALIVE get_wasm_audio_buf(void) { return (uint8_t *)wasmbuf; }
uint8_t *EMSCRIPTEN_KEEPALIVE get_wasm_preset_buf(void) { return (uint8_t *)&rampreset; }
...
int EMSCRIPTEN_KEEPALIVE wasm_getcurpreset(void) { return sysparams.curpreset; }
void EMSCRIPTEN_KEEPALIVE wasm_setcurpreset(int i) { SetPreset(i, false); }
#endif
...
bool serial_midi_ready(void) { return huart3.TxXferCount == 0; }
void kick_midi_send(void) {
...
#else
void kick_midi_send(void) {}
bool serial_midi_ready(void) { return true; }
#endif
